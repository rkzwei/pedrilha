...
use gem_finder_shared::{
    ...
    types::{
        Movie, TmdbConfig, TmdbCredits, TmdbDiscoverResponse, TmdbFindResponse,
        TmdbKeywordsResponse, TmdbMovieDetail,
    },
};
use reqwest::Client;
...
impl TmdbSyncService {
    ...
    async fn sync_single_movie(&self, conn: &Connection, tmdb_id: i64) -> Result<i64> {
        ...
        let keywords_url = format!(
            "{}/movie/{}/keywords?api_key={}",
            self.base_url, tmdb_id, self.api_key
        );
        let keywords_resp: TmdbKeywordsResponse =
            self.client.get(&keywords_url).send().await?.json().await?;
        let keywords_str = keywords_resp
            .keywords
            .iter()
            .map(|k| k.name.clone())
            .collect::<Vec<_>>()
        ...
        let movie = Movie {
            ...
            keywords: if keywords_str.is_empty() {
                None
            } else {
                Some(keywords_str)
            },
