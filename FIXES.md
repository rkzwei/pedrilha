...
| FIX-17 | Gem score ceiling too low | ✅ done — `run_batch_scoring` now normalizes: after sorting, divides all scores by `scored[0].score` (population max), so the top gem in any run = 100%. Logs the pre-normalization max. |
| FIX-18 | UI — no styling, single-column list | ✅ done — full UI: 5-col responsive grid, filter bar (decade + genre), pagination controls, skeleton loading cards, gem-score overlay badge, IMDb/RT badges on cards, director display, hover ring. `AcclaimedPage` added. `Effect::new` replaces manual `spawn_local` + closure sharing for reactive re-fetch on filter/page change. |
| FIX-19 | Automated sync not scheduled | ✅ done — `main.rs` runs a background tokio task every `SYNC_INTERVAL_HOURS` (default 24) chaining sync → enrich → score → classify → provider sync → cache invalidate, guarded by `admin_busy`/`BusyGuard` |
...
| FIX-43 | Era-window sort still wrong — `sort_by=primary_release_date.desc` within each 5-page window means window (2012, None) only returns 2023 films (100 most recently released); 2013-2020 films fall through the gap between windows | ✅ done — changed sort to `vote_count.desc` in `sync_movies`; each window now returns the most-notable films distributed across the full era. Also updated `admin.rs` `trigger_sync` to use same 4 era-window pattern (was calling `sync_movies` with a single window). |
| FIX-44 | The Messenger (2009, tt1340803) missing from SEEDED_GEMS | ✅ done — added to `SEEDED_GEMS` in `constants.rs`. |
| FIX-45 | `sync_movies` + `sync_acclaimed_candidates` capped at 5 pages — misses majority of candidates | ✅ done — removed `MAX_PAGES = 5` cap; both functions now exhaust `response.total_pages` (up to TMDB's 500-page ceiling). Added wave-based rate limiting: `PAGE_DELAY_MS=300ms` between every page, `WAVE_DELAY_MS=2000ms` every 20 pages (`WAVE_SIZE`). Constants in `constants::tmdb_rate_limit`. Re-run safe: cached movies skip `sync_single_movie`, so only discover calls fire on subsequent runs. |
| FIX-46 | `<A class=` Leptos compile error in `main.rs` and `pages.rs` | ✅ done — `<A>` is a component in Leptos 0.7, not an HTML element; HTML attrs need `attr:` prefix (`attr:class=`). Fixed 4 occurrences in `main.rs`, 3 in `pages.rs`. |
| FIX-47 | Pagination buttons rendered as raw `\|\|` text — `disabled=move \|\|` confused `view!` macro | ✅ done — hoisted to `let prev_disabled = page.get() <= 1;` / `let next_disabled = page.get() >= tp;` before the `view!` block. |
| FIX-48 | `SkeletonCard` component used but never defined; `web_sys` import missing in `pages.rs` | ✅ done — added `SkeletonCard` loading placeholder with `animate-pulse`; added `use leptos::web_sys`. |
| FIX-49 | `MovieCard` `view!` macro unclosed — missing `</A>`, closing `}` for `view!`, closing `}` for function | ✅ done — code was truncated at the end of the div; inserted `</A>\n    }\n}` to close correctly. Manifested as "unclosed delimiter" at EOF. |
| FIX-50 | CI failing — `npm ci` requires `package-lock.json` (gitignored); `wasm-bindgen-cli --locked` failed on rustc version chain | ✅ done — changed `npm ci` → `npm install`; removed `--locked` flag from `cargo install wasm-bindgen-cli`. |
| FIX-51 | Docker build context 22 GB — `target/` not excluded from build context | ✅ done — added `.dockerignore` excluding `target/`, `**/target/`, `node_modules/`, `dist/`, `SECRETS.env`, `.git/`, `*.db`, `*.log`. |
| FIX-52 | Docker trunk "could not find root package" in `frontend-builder` stage | ✅ done — trunk run from workspace root can't resolve the frontend package via `trunk.toml`. Fixed: `RUN cd crates/frontend && trunk build --release`. Matches CI `working-directory: crates/frontend`. |
| FIX-53 | Docker `frontend-builder` produced silent `fn main(){}` binary — dep-cache layer mtime collision | ✅ done — Docker BuildKit preserves host file timestamps on `COPY`. Dep-cache dummy sources have current-build-time mtime; COPIEd real sources have older host mtime. Cargo sees "source older than artifact" → skips recompile → dummy binary exits with code 0. Fixed: `find ... -exec touch {} +` before final `cargo build`. |
| FIX-54 | `docker-compose.yml` in `docker/` subdirectory — `docker compose` could not find config from repo root | ✅ done — moved to repo root; updated paths: `context: .`, `dockerfile: docker/Dockerfile`, `env_file: ./SECRETS.env`. |
| FIX-55 | Docker container used Turso remote — `env_file` passed `TURSO_DATABASE_URL` into container despite local-only requirement | ✅ done — `environment:` section overrides with `TURSO_DATABASE_URL=`, `TURSO_AUTH_TOKEN=`, `DATABASE_URL=` (empty). Hardened `from_env()` in `db/connection.rs` to `.filter(\|s\| !s.is_empty())` so empty string is treated as unset. |
| FIX-56 | API keys read per-request from env; missing key produced cryptic 500 with no message | ✅ done — keys moved into `AppState` (`tmdb_api_key: String`, `omdb_api_key: String`); read at startup with `tracing::warn!` if empty (server still starts); `TmdbSyncService::new(key)` and `OmdbEnrichmentService::new(key)` now accept key directly (infallible); admin endpoints return `503 SERVICE_UNAVAILABLE` if key is empty. |
| FIX-57 | Admin ops cancelled on browser tab close — handlers awaited long work inline; Axum drops handler future on client disconnect | ✅ done — all three handlers (`trigger_sync`, `trigger_enrich`, `trigger_score`) spawn `tokio::spawn` background tasks, return 202 immediately. "Keep page open" warning removed from UI; replaced with "Operations run on the server — you can close this page." |
| FIX-58 | OMDb enrichment hardcoded at 500 movies — OMDb API allows 100,000 requests/day | ✅ done — user-passable limit input on admin panel (range 1–50,000, default 10,000); backend default raised to 10,000. |
| FIX-59 | Database lost on every Docker rebuild — DB lived inside container image | ✅ done — named Docker volume `gem-data:/app` persists across `docker compose build` and `up/down`. Only deleted on `docker compose down -v`. |
| FIX-60 | Docs drifted from code across multiple phases (Leptos 0.7→0.8 never updated post-upgrade, `ARCHITECTURE.md`'s `watchlist` schema table still showed the Phase-1 placeholder dropped in migration v2, `movie_providers`/`provider_sync`/`user_providers`/Phase 10 routes mislabeled "planned" despite shipping, algorithm weights stale since Phase 6, Phase 8/9/10 deliverable checklists left unchecked despite Status callouts saying shipped, `GEM_FINDER_MEMORY.md` asserted a public watchlist-rating aggregate that was never built) | ✅ done — full pass against `Cargo.toml`, `constants.rs`, `migrations.rs`, `main.rs` routes/`AppState`, and `crates/frontend/src/*` on 2026-07-05; `ARCHITECTURE.md`, `PHASES.md`, `README.md`, `GEM_FINDER_MEMORY.md` corrected with verified ground truth, not guesses. See FIX-11 for the earlier instance of this same class of bug — recommend a lighter periodic doc-vs-code diff rather than only at phase boundaries. |
